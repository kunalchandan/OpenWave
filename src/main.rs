...
pub fn stream_setup_for(frequency: f32) -> Result<cpal::Stream, anyhow::Error>
where
{
    let (_host, device, config) = host_device_setup()?;

    match config.sample_format() {
        cpal::SampleFormat::I8 => make_stream::<i8>(&device, &config.into(), frequency),
        cpal::SampleFormat::I16 => make_stream::<i16>(&device, &config.into(), frequency),
        cpal::SampleFormat::I32 => make_stream::<i32>(&device, &config.into(), frequency),
        cpal::SampleFormat::I64 => make_stream::<i64>(&device, &config.into(), frequency),
        cpal::SampleFormat::U8 => make_stream::<u8>(&device, &config.into(), frequency),
        cpal::SampleFormat::U16 => make_stream::<u16>(&device, &config.into(), frequency),
        cpal::SampleFormat::U32 => make_stream::<u32>(&device, &config.into(), frequency),
        cpal::SampleFormat::U64 => make_stream::<u64>(&device, &config.into(), frequency),
        cpal::SampleFormat::F32 => make_stream::<f32>(&device, &config.into(), frequency),
        cpal::SampleFormat::F64 => make_stream::<f64>(&device, &config.into(), frequency),
    ...
    T: SizedSample + FromSample<f32>,
{
    ...
    let stream = device.build_output_stream(
        ...
        move |output: &mut [T], _: &cpal::OutputCallbackInfo| {
            ...
            } else if time_since_start < 8.0 {
                oscillator.set_waveform(Waveform::Triangle);
            } else if time_since_start < 12.0 {
                oscillator.set_waveform(Waveform::Square);
            } else if time_since_start < 16.0 {
...
fn main() -> anyhow::Result<()> {
    ...
    let f_sampling: u32 = 48_000; // CD frequency
    let s_lut: Vec<f32> = (0..f_sampling)
        .map(|x| ((x as f32) * 2.0 * PI / (f_sampling as f32)).sin())
        .collect();

    let frequency = 69.0;
    let stream = stream_setup_for(frequency)?;
    stream.play()?;
